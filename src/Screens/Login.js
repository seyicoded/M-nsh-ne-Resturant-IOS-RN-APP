        ...
        const asyncLoader = async ()=> {
            ...
            if(validateEmail(email)){
                if(password.length >4){
                    setisLoading(true)
                    const fd = {"email": email, "password": password}
                    //console.log(fd)
                    const fetchLoginRequestData = await axios.post(BASE_URL+'login',fd)
                            ...
                            <View style={styles.content_holder}>
                                ...
                                <View style={{width: '90%'}}>
                                    ...
                                    <View>
                                        ...
                                        <TextInput value={email} autoCapitalize="none" returnKeyType='next' placeholder="Enter Your Email" textContentType="emailAddress" keyboardType="email-address" placeholderTextColor='rgba(255,255,255,0.7)' onChangeText={(txt=>setemail(txt))} style={{...styles.input_design}}/>
                                    </View>

                                    <View>
                                        <Icon1 name="key" value={password} color="darkblue" style={styles.form_text_image}/>
                                        <TextInput autoCapitalize="none" placeholder="Enter Your Password" placeholderTextColor='rgba(255,255,255,0.7)' style={{...styles.input_design}} onChangeText={(txt=>setpassword(txt))} secureTextEntry={true}/>
                                    </View>

                                    <TouchableOpacity onPress={
                                        ()=>{
                                            navigation.navigate('stack_screen_forgot_password')
                                        }
                                    }>
                                        <Text style={{color: 'white', fontWeight: 'bold', textAlign: 'right'}}>Forgot Password?</Text>
                                    </TouchableOpacity>
                                    <Text />

                                    <TouchableOpacity onPress={()=>{setisLoading(true);SignIn();}}>
                                        <Text style={styles.form_submit}>LOG IN</Text>
                                    </TouchableOpacity>
                                    
                                    <Text style={{textAlign: 'center', margin: 10, color: 'white'}}>OR</Text>
                                    
                                    <View style={{flex: 1,flexDirection: 'row', width: '100%', justifyContent: 'space-evenly'}}>
                                        <TouchableOpacity>
                                            <View style={{...styles.form_submit,flex: 1, width: '100%', backgroundColor: 'blue', padding: 10, paddingHorizontal: 30}}>
                                                <Icon2 name='facebook' style={{fontSize: 38, color: 'white'}}/>
                                            </View>
                                        </TouchableOpacity>
                                        <TouchableOpacity>
                                            <View style={{...styles.form_submit,flex: 1, width: '100%', backgroundColor: 'red', padding: 10, paddingHorizontal: 30,}}>
                                                <Icon2 name='google' style={{flex:1, color: 'white', fontSize: 33, shadowColor: 'red'}}/>
                                            </View>
                                        </TouchableOpacity>
                                    </View>
                                ...
                                <TouchableOpacity onPress={
                                    ()=>{
                                        navigation.navigate('stack_screen_register')
                                    }
                                }>
                                    <Text style={{color: 'white', fontWeight: 'bold', textAlign: 'right'}}>SIGN UP</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                        </ScrollView>
